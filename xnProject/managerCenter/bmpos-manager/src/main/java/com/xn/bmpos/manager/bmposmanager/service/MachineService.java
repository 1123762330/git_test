    ...
    public List<MachineWebModel> querMergeAll (String path) {
        ...
        if (machineWebModels.size() > 0) {
            ...
            for (MachineWebModel machineWebModel : machineWebModels) {
                ...
                double nissan = 0.0;
                //根据币种 去查询数据库配置
                Setting setting = settingService.selectCoin(machineWebModel.getCoinTypeName());
                //setting表中数据
                double bockPrice = 0;
                String fee = null;
                //全网难度
                ...
                if (setting != null) {
                    //发送请求获取redis中数据
                    String str = OkHttpUtils.get(setting.getRpchost() + path, null);
                    //判断请求是否有返回值
                        ...
                        for (int i = 0; i < array.size(); i++) {
                            ...
                            if (setting.getName().equals(indexCoinModel.getCoin().toLowerCase())) {
                                ...
                                if ("ltc".equals(setting.getName())) {
                                    ...
                                    nissan = hashrate * 0.001 * earnings * 1.4925044 * Double.parseDouble(resultMap.get("ltc")) * 6.82;
                                }
                            }
                        }
                        if (nissan == 0.0) {
                            if ("btc".equals(setting.getName())) {
                                ...
                                nissan = hashrate * earnings * 1.023 * Double.parseDouble(resultMap.get("btc")) * 6.82;
                            }
                        }
                    } else {
                    ...
                    System.out.println("setting表查询为空");
                }
                if (nissan == 0.0) {
                    nissan = 80.0;
                }
