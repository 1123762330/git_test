...
<script>
            ...
            "success": function (json) {
                if (json.state == 200) {
                    ...
                    var table = ' <tr>\n' +
                        ...
                        ' <th width="70px"> 用户时长</th>\n' +
                        ' <th width="70px"> 矿机天数</th>\n' +
                        ' <th width="70px"> 矿机时长</th>\n' +
                        ' </tr>';
                    user_lis += table;//将第一行标题追加到页面上

                    for (var i = 0; i < worksData.length; i++) {
                        var workData = worksData[i];
                        var puid = workData.id;
                        var shuju = '<tr>\n' +
                            '<th>' + workData.id + '</th>' +
                            '<th>' + workData.username + '</th>' +
                            '<th>' + workData.pass + '</th>' +
                            '<th>' + workData.email + '</th>' +
                            '<th>' + '<a href="#" onclick="searchUserDay(' + puid + ')">查看</a>' + '</th>' +
                            '<th>' + '<a href="#" onclick="searchUsersHour(' + puid + ')">查看</a>' + '</th>' +
                            '<th>' + '<a href="#" onclick="searchWorkersDay(' + puid + ')">查看</a>' + '</th>' +
                            '<th>' + '<a href="#" onclick="searchWorkersHour(' + puid + ')">查看</a>' + '</th>' +
                            '</tr>';
                ...
                if (json.state == 200) {
                    ...
                    for (var i = 0; i < poolData.length; i++) {
                        ...
                        var shuju = '<tr>\n' +
                            ...
                            '<th>' + pool.earn + '</th>' +
                            '<th>' + pool.lucky + '</th>' +
                            '<th>' + renderTime(pool.createdAt) + '</th>' +
                            '<th>' + renderTime(pool.updatedAt) + '</th>' +
                            '</tr>';
    ...
    //统计用户在线天数
    function searchUserDay(puid) {
        $.ajax({
            "url": "${pageContext.request.contextPath}/findUserDay?puid=" + puid,
            "type": "GET",
            "dataType": "json",
            "success": function (json) {
                if (json.state == 200) {
    ...
    //统计用户在线小时
    function searchUsersHour(puid) {
        $.ajax({
            "url": "${pageContext.request.contextPath}/findUsersHour?puid=" + puid,
            "data": '',
            "type": "GET",
            "dataType": "json",
            "success": function (json) {
    ...
    //统计矿机在线天数
    function searchWorkersDay(puid) {
        $.ajax({
            "url": "${pageContext.request.contextPath}/findWorkersDay?puid="+puid,
            "data": '',
            "type": "GET",
            "dataType": "json",
            "success": function (json) {
                if (json.state == 200) {
                    ...
                    for (var i = 0; i < workersDayData.length; i++) {
                        ...
                        var shuju = '<tr>\n' +
                            ...
                            '<th>' + renderTime(workersDay.createdAt) + '</th>' +
                            '<th>' + renderTime(workersDay.updatedAt) + '</th>' +
                            '</tr>';
    ...
    //统计矿机在线小时
    function searchWorkersHour(puid) {
        $.ajax({
            "url": "${pageContext.request.contextPath}/findWorkersHour?puid="+puid,
            "data": '',
            "type": "GET",
            "dataType": "json",
            "success": function (json) {
                if (json.state == 200) {
                    ...
                    for (var i = 0; i < workersHourData.length; i++) {
                        ...
                        var shuju = '<tr>\n' +
                            ...
                            '<th>' + renderTime(workersHour.createdAt) + '</th>' +
                            '<th>' + renderTime(workersHour.updatedAt) + '</th>' +
                            '</tr>';
                        list += shuju;//将数据都添加到列表中
                    }
                    //将数据列表添加到页面上
